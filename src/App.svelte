...
<div class="studio-shell" class:project-open={projectOpen}>
  <header class="studio-topbar">
    ...
    <div class="topbar-status"><span class:invalid={!studio.validation.ok}>{studio.validation.ok ? '✓ SPIELBAR' : `⚠ ${studio.validation.errors.length} FEHLER`}</span><span class:cloud-warning={studio.cloudStatus === 'conflict' || studio.cloudStatus === 'offline'} class="cloud-status">{studio.cloudStatus === 'shared' ? '☁ GEMEINSAM' : studio.cloudStatus === 'syncing' ? '☁ SYNCHRONISIERT …' : studio.cloudStatus === 'conflict' ? '⚠ CLOUD-KONFLIKT' : studio.cloudStatus === 'offline' ? '○ NUR LOKAL' : studio.saveStatus}</span><button onclick={() => studio.undo()} disabled={!studio.canUndo} title="Rückgängig (Strg+Z)">↶</button><button onclick={() => studio.redo()} disabled={!studio.canRedo} title="Wiederholen (Strg+Y)">↷</button></div>
