...
<div class="studio-shell" class:project-open={projectOpen}>
  <header class="studio-topbar">
    ...
    <div class="document-identity"><span>{studio.level.icon}</span><div><strong>{studio.level.name.standard}</strong><small>{studio.level.location.area} · {studio.level.board.columns}×{studio.level.board.rows}</small></div></div>
  ...
  <aside class="project-drawer" id="project-drawer" aria-label="Projekt und Vorlagen">
    ...
    <div class="project-section"><div class="panel-title"><strong>Passau-Vorlagen</strong><span>{templates.length}</span></div><div class="template-list">{#each templates as level}<button class:active={studio.level.id === level.id} data-template-id={level.id} onclick={() => loadTemplate(level.id)}><span>{level.icon}</span><div><strong>{level.name.standard}</strong><small>{level.location.area} · {level.board.columns}×{level.board.rows}</small></div></button>{/each}</div></div>
    <div class="project-section"><div class="panel-title"><strong>Meine Entwürfe</strong><span>{drafts.length}</span></div>{#if drafts.length}<div class="draft-list">{#each drafts as draft}<button class:active={studio.level.id === draft.id} onclick={() => loadDraft(draft.id)}><span>◫</span><div><strong>{draft.name}</strong><small>{new Date(draft.savedAt).toLocaleString('de-DE', { dateStyle: 'short', timeStyle: 'short' })}</small></div></button>{/each}</div>{:else}<p class="hint">Änderungen erscheinen nach dem ersten Speichern automatisch hier.</p>{/if}</div>
