...
<section class="effect-editor" aria-label={title}>
  ...
  {#each effects as effect, index}
    <div class="effect-card" data-effect-type={effect.type}>
      ...
      <div class="effect-fields"><label>Stärke<input aria-label={`Effekt ${index + 1} Stärke`} type="range" min="0.05" max="1" step="0.05" value={effect.intensity} oninput={(event) => update(index, 'intensity', Number(event.currentTarget.value))} /><output>{Math.round(effect.intensity * 100)}%</output></label><label>Tempo<input aria-label={`Effekt ${index + 1} Tempo`} type="range" min="0.1" max="8" step="0.1" value={effect.speed} oninput={(event) => update(index, 'speed', Number(event.currentTarget.value))} /><output>{effect.speed.toFixed(1)}×</output></label><label>Farbe<input aria-label={`Effekt ${index + 1} Farbe`} type="color" value={effect.color} onchange={(event) => update(index, 'color', event.currentTarget.value)} /></label></div>
    </div>
  {/each}
</section>
